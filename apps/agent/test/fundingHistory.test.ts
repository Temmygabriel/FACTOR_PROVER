...
describe('pagination', () => {
  it('stops once a page’s OLDEST settlement is at or before earliestMs', () => {
  ...
  it('stops on the FIRST page when that page already reaches the horizon', () => {
    const { urls } = stubPages([[row(T(2), 0.00002), row(T(1), 0.00001)]]);
    await fetchFundingHistory('BTCUSDT', T(2));
    expect(urls).toHaveLength(1);
  });

  it('walks pages 1, 2, 3 … in order, with the measured query shape', () => {
  ...
  it('stops on an empty page rather than paging on forever', () => {
  ...
  it('stops at the page cap instead of paging without limit', () => {
...
describe('the returned series', () => {
  ...
  it('returns ASCENDING order even though the wire is newest-first', () => {
  ...
  it('parses both fields to numbers, since the API sends strings', () => {
  ...
  it('tolerates a response with no data field', () => {
...
describe('failures', () => {
  it('throws after exhausting the retry attempts on a network error', () => {
  ...
  it('does not retry when attempts is 1', () => {
    const fn = vi.fn(async () => {
      throw new Error('ECONNRESET');
    });
    vi.stubGlobal('fetch', fn);
  ...
  it('rides out a 5xx and succeeds on the next attempt', () => {
    let call = 0;
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => {
  ...
  it('gives up on a 5xx once the attempts are spent', () => {
  ...
  it('also ends in a throw on a 4xx, after the same number of attempts', () => {
